...
Key Differences Between NVIDIA GB300 and B300 GPUs (Per GPU)

- **Compute:** GB300 dense **FP4** performance is **7.1% faster** (15 vs 14 PFLOPS), FP8 and FP16/BF16 is **+11.1%**. TF32 is also slightly faster with speedup of **13.64%** 
- **Memory capacity and bandwidth:** GB300 provides slightly higher HBM3E capacity (**279 vs 270 GB**, +3.3%) and HBM bandwidth (**8 vs 7.7 TB/s**, +3.9%).
- **Power headroom and clocks:** GB300 has a higher max GPU TDP (**1.4 kW vs 1.1 kW**, +27.3%), which increases headroom to sustain higher SM/Tensor-Core clocks during long GEMM-heavy phases.
- **B200/B300/GB300 GEMM performance):** Part of the practical GEMM throughput gain from **B200 -> B300** (1.0 -> 1.1 kW) and further to **GB300** (1.4 kW) comes from power headroom that lets boost clocks stay higher for longer. GPUs are unable to sustain their peak clock speed due to power throttling thus increase in power in does also translate in higher achievable performance.
...
GB300 NVL72 includes **36 Grace CPUs** and 72 GPUs (`2 GPUs per Grace CPU`). This creates a coherent/fabric-accessible CPU-memory tier that is capacity-focused compared to local GPU HBM.
...
Unified/coherent memory tiers in practice:
- `Tier 0 (local GPU HBM3e)`: 279 GB and 8 TB/s per GPU (fastest tier).
- `Tier 0.5 (remote GPU HBM over global unfied NVLink)`: HBM capacity is reachable across NVLink fabric, but bandwidth is fabric-limited (not 8 TB/s local-HBM speed).
- `Tier 1 (Grace LPDDR5X, coherent/fabric-accessible)`: larger shared capacity tier (17 TB rack total), but with much lower sustained bandwidth than local HBM.
...
2. [NVIDIA Blackwell Ultra Datasheet](https://resources.nvidia.com/en-us-blackwell-architecture/blackwell-ultra-datasheet?ncid=no-ncid)
3. [NVIDIA GB300 NVL72 Platform](https://www.nvidia.com/en-us/data-center/gb300-nvl72/)
4. [NVIDIA Grace Performance Tuning Guide](https://docs.nvidia.com/dccpu/grace-perf-tuning-guide/index.html)
5. [NVIDIA NVLINK Multi-Node Tuning Guide, NCCL](https://docs.nvidia.com/multi-node-nvlink-systems/multi-node-tuning-guide/nccl.html)
6. [Introducing NVFP4 for Efficient and Accurate Low-Precision Inference](https://developer.nvidia.com/blog/introducing-nvfp4-for-efficient-and-accurate-low-precision-inference/)
7. [NVIDIA DGX B300 Datasheet](https://resources.nvidia.com/en-us-dgx-systems/dgx-b300-datasheet)
8. [NVIDIA DGX B200 Datasheet](https://resources.nvidia.com/en-us-dgx-systems/dgx-b200-datasheet)
9. [Strangely, Matrix Multiplications on GPUs Run Faster When Given "Predictable" Data! (Horace He)](https://www.thonking.ai/p/strangely-matrix-multiplications)
