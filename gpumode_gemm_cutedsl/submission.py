...
class GEMMKERNEL:
    ...
    def __call__(
        ...
        self.buffer_align_bytes = 1024 # 1024
    ...
    def _compute_stages(
        ...
        pos = (smem_capacity // occupancy - (mbar_helpers_bytes + c_bytes)) // ab_bytes_per_stage

        num_ab_stage = max(
            pos,
            5  # Minimum stages to hide TMA latency
        )
        ...
        #num_c_stage = min(num_c_stage, 2)

        #print(f"num_ab_stage={num_ab_stage}, num_acc_stage={num_acc_stage}, num_c_stage={num_c_stage}")
    ...
    def can_implement(
        ...
        if not GEMMKERNEL.is_valid_dtypes_and_scale_factor_vec_size(
            ab_dtype, sf_dtype, sf_vec_size, c_dtype
        ):
            can_implement = False
        # Skip unsupported layouts
        if not GEMMKERNEL.is_valid_layouts(
            ab_dtype, c_dtype, a_major, b_major, c_major
        ):
            can_implement = False
        # Skip invalid mma tile shape and cluster shape
        if not GEMMKERNEL.is_valid_mma_tiler_and_cluster_shape(
            mma_tiler_mn, cluster_shape_mn
        ):
            can_implement = False
        # Skip illegal problem shape for load/store alignment
        if not GEMMKERNEL.is_valid_tensor_alignment(
...
c_dtype = cutlass.Float16          # FP16 for output


_compiled_kernel_cache = {}  # Cache per problem configuration
...
def get_optimal_config(m, n, k):
  ...
  - {"m": 128, "n": 7168, "k": 2048, "l": 1, "seed": 1111}
    """
    #if m <= 128:
    #    # Smaller tile = more SMEM for stages
    #    return (128, 64), (2, 1)
    #elif m <= 256:
    #    return (128, 128), (2, 1)
    return (128, 64), (1, 1)
...
def compile_kernel(m, n, k):
    global _compiled_kernel_cache

    # Create cache key based on problem size category
    config_key = get_optimal_config(m, n, k)

    if config_key in _compiled_kernel_cache:
        return _compiled_kernel_cache[config_key]

    mma_tiler, cluster_shape = config_key

    kernel_instance = GEMMKERNEL(
        sf_vec_size=sf_vec_size,
        mma_tiler_mn=mma_tiler,
        cluster_shape_mn=cluster_shape,
    )

    # Set the kernel instance globally BEFORE compilation
    # This is needed because cute.compile() will execute the function to build IR
    global _current_gemm_kernel
    _current_gemm_kernel = kernel_instance

    # Compute max active clusters for this configuration
    # This gets baked into the compiled code (Constexpr parameter)
    hardware_info = cutlass.utils.HardwareInfo()
    max_active_clusters = hardware_info.get_max_active_clusters(
        cluster_shape[0] * cluster_shape[1]
    )

    # Create pointers with null addresses for compilation
    ...
    compiled_func = cute.compile(
        ...
        sfb_ptr,
        c_ptr,
        (0, 0, 0, 0),  # Dummy problem size for compilation
        max_active_clusters  # Baked into compiled code as Constexpr
    )

    # Cache: compiled function, kernel instance, and cluster_shape (for hardware_info)
    _compiled_kernel_cache[config_key] = (compiled_func, kernel_instance, cluster_shape)

    return compiled_func, kernel_instance, cluster_shape


_current_gemm_kernel = None
_printed_sizes = set()  # Track which problem sizes have been printed

def _get_current_kernel():
    global _current_gemm_kernel
    return _current_gemm_kernel
...
def _kernel_wrapper(
    ...
    # Create CuTe tensors with full layout information
    # A tensor: M × K × L (row-major in K)
    ...
    # SFA tensor: Create layout from permuted scale factors
    sfa_layout = blockscaled_utils.tile_atom_to_shape_SF(a_tensor.shape, sf_vec_size)
    sfa_tensor = cute.make_tensor(sfa_ptr, sfa_layout)

    # SFB tensor: Create layout from permuted scale factors
    sfb_layout = blockscaled_utils.tile_atom_to_shape_SF(b_tensor.shape, sf_vec_size)
    sfb_tensor = cute.make_tensor(sfb_ptr, sfb_layout)

    gemm = _get_current_kernel()

    epilogue_op = lambda x: x  # Identity function (no-op epilogue)
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Print problem size only once per unique (m, k, n)
    #global _current_gemm_kernel, _printed_sizes
    #size_key = (m, k, n)
    #if size_key not in _printed_sizes:
    #    print(f"Analyzing m:{m} k:{k} n:{n}")
    #    _printed_sizes.add(size_key)
    compiled_func, _current_gemm_kernel, cluster_shape = compile_kernel(m, n, k)
