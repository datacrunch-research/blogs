...
HOST_MODEL_PATH="/mnt/local_nvme/models"
C_MODEL_PATH="/root/models"
...
#Model downloader
docker run --gpus all --shm-size 32g --network=host --name model_downloader -it --rm --ipc=host \
    -v $HOST_MODEL_PATH:$C_MODEL_PATH \
    -e HF_TOKEN=$HF_TOKEN \
    lmsysorg/sglang:dev-cu13 \
    huggingface-cli download nvidia/DeepSeek-R1-0528-NVFP4-v2 \
        --cache-dir $C_MODEL_PATH

# launch server
docker run --gpus all --shm-size 32g --network=host --name model_downloader -it --ipc=host \
    -v $HOST_MODEL_PATH:$C_MODEL_PATH \
    -e HF_TOKEN=$HF_TOKEN \
    lmsysorg/sglang:dev-cu13 \
